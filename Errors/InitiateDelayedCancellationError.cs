...
public sealed class InitiateDelayedCancellationError : ApiError
{
    ...
    internal static Task<InitiateDelayedCancellationError> Create(HttpResponseMessage response,
...
internal sealed class InitiateDelayedCancellationErrorResponse : IErrorResponse<InitiateDelayedCancellationError>
{
    public static InitiateDelayedCancellationErrorResponse Instance { get; } = new();

    private InitiateDelayedCancellationErrorResponse()
    {
    }

    public Task<InitiateDelayedCancellationError> Map(HttpResponseMessage response, CancellationToken ct) =>
        InitiateDelayedCancellationError.Create(response, ct);
}
