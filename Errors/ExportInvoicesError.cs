...
public sealed class ExportInvoicesError : ApiError
{
    ...
    internal static Task<ExportInvoicesError> Create(HttpResponseMessage response, CancellationToken ct) =>
...
internal sealed class ExportInvoicesErrorResponse : IErrorResponse<ExportInvoicesError>
{
    public static ExportInvoicesErrorResponse Instance { get; } = new();

    private ExportInvoicesErrorResponse()
    {
    }

    public Task<ExportInvoicesError> Map(HttpResponseMessage response, CancellationToken ct) =>
        ExportInvoicesError.Create(response, ct);
}
