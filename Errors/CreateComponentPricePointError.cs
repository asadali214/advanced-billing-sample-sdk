...
public sealed class CreateComponentPricePointError : ApiError
{
    ...
    internal static Task<CreateComponentPricePointError> Create(HttpResponseMessage response, CancellationToken ct) =>
...
internal sealed class CreateComponentPricePointErrorResponse : IErrorResponse<CreateComponentPricePointError>
{
    public static CreateComponentPricePointErrorResponse Instance { get; } = new();

    private CreateComponentPricePointErrorResponse()
    {
    }

    public Task<CreateComponentPricePointError> Map(HttpResponseMessage response, CancellationToken ct) =>
        CreateComponentPricePointError.Create(response, ct);
}
